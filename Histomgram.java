...
public class Histogram extends Application {
	...
	public void start(Stage primaryStage) {
		...
		hBox.getChildren().addAll(new Label("Key (in double)"), tfKey, btStep, btReset);
		...
		btStep.setOnAction(e -> {
			...
			if (status == 0)
				...
				lblStatus.setText("The key is found in the array at index " + control.getCurrentIndex());
			} else if (status == -1)
		...
		public void repaint() {
			...
			for (int i = 0; i < numbers.length; i++) {
				...
				this.getChildren().add(new Text(i * unitWidth + 10 + 10,
						getHeight() - (numbers[i] * 1.0 / max) * height - 10, numbers[i] + ""));
			}
